...
const paranoia: Command = {
  ...
  run: async (ctx: Context): Promise<void> => {
    ...
    if (!status.guildOpen) {
      ...
      const targetUserId = (ctx.getOption('target') as ApplicationCommandInteractionDataOptionUser)?.value
      ...
      const messageResponse = await superagent
        ...
        .send({
          embeds: [{
            ...
            fields: [{
              name: " ",
              value: paranoia.question
            }],
            footer: { text: `Type: ${paranoia.type} | Rating: ${paranoia.rating} | ID: ${paranoia.id}` }
          }]
